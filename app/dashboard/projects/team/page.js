...
import { Checkbox } from "@/components/ui/checkbox";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { Loader2, Users2, UserPlus, Users, Send, RefreshCcw } from "lucide-react";
...
export default function TeamAndPartnersPage() {
  ...
  // Şirket çalışanları (global liste; sağ sütun için filtrelenecek)
  const [companyMembers, setCompanyMembers] = useState([]);
  const [membersLoading, setMembersLoading] = useState(false);

  // Seçili projedeki üyeler (sol sütun)
  const [projectMembers, setProjectMembers] = useState([]);
  const [projectMembersLoading, setProjectMembersLoading] = useState(false);

  // Sağ sütunda seçimler
  const [selectedMemberIds, setSelectedMemberIds] = useState([]);

  // Partner şirketler
  const [partnerCompanies, setPartnerCompanies] = useState([]); // sol sütun
  const [partnersLoading, setPartnersLoading] = useState(false);

  // Davet edilebilir şirketler (sağ sütun)
  const [companies, setCompanies] = useState([]);
  const [companiesLoading, setCompaniesLoading] = useState(false);
  const [selectedCompanyIds, setSelectedCompanyIds] = useState([]);
  ...
  const refreshProjects = async () => {
    ...
    try {
      ...
      // B) iki ayrı sorgu → birleştir
      const promises = [];
      if (myCompanyId) promises.push(supabase.from("projects").select("id, name").eq("company_id", myCompanyId));
      if (memberProjectIds.length) promises.push(supabase.from("projects").select("id, name").in("id", memberProjectIds));
  ...
  const refreshMembers = async () => {
    ...
    try {
      ...
      const mapped = (rows || []).map((r) => ({
        user_id: r.user_id,
        role: r.role ?? null,       // 'patron' | 'yonetici' | 'calisan'
        email: r.email ?? null,
        full_name: r.full_name ?? null,
      }));
  ...
  /* ----------------------- PROJE ÜYELERİ (sol sütun) ---------------------- */
  const refreshProjectMembers = async () => {
    if (!selectedProjectId || !myCompanyId) {
      setProjectMembers([]);
      return;
    }
    ...
    setError(null);
    try {
      // Bu proje + bu şirket üyeleri
      const { data, error } = await supabase
        .from("project_members")
        .select("user_id, role, status, users:auth.users(email)")
      ...
      console.error(e);
      setError(`Proje üyeleri yüklenemedi: ${e?.message || e}`);
    } finally {
      setProjectMembersLoading(false);
    }
  };
  useEffect(() => { refreshProjectMembers(); /* eslint-disable-line react-hooks/exhaustive-deps */ }, [selectedProjectId, myCompanyId]);

  /* ----------------------- PARTNER ŞİRKETLER ---------------------- */
  const refreshPartnerCompanies = async () => {
    ...
    setError(null);
    try {
      // Projedeki tüm şirketleri getir, kendi şirketini çıkar, tekilleştir
      const { data, error } = await supabase
        .from("project_members")
        .select("company_id, company:company(name)")
        .eq("project_id", selectedProjectId)
        .eq("status", "active");
      if (error) throw error;
      ...
      for (const r of data || []) {
        ...
        if (!cid || (myCompanyId && cid === myCompanyId)) continue;
        if (!setMap.has(cid)) {
          setMap.set(cid, { id: cid, name: r?.company?.name || "Bilinmeyen" });
        }
      }
      setPartnerCompanies(Array.from(setMap.values()).sort((a, b) => a.name.localeCompare(b.name)));
    } catch (e) {
      console.error(e);
      setError(`Partner şirketler yüklenemedi: ${e?.message || e}`);
    } finally {
      setPartnersLoading(false);
    }
  };
  useEffect(() => { refreshPartnerCompanies(); /* eslint-disable-line react-hooks/exhaustive-deps */ }, [selectedProjectId, myCompanyId]);

  /* ----------------------- DAVET EDİLEBİLİR ŞİRKETLER (sağ) ---------------------- */
  const refreshCompanies = async () => {
    setCompaniesLoading(true);
    setError(null);
    try {
      // Mevcut partner şirket ID set'i
      const existingPartnerIds = new Set(partnerCompanies.map((c) => c.id));
      // Şirket listesi (kendi şirketin hariç)
      let q = supabase.from("company").select("id, name");
      if (myCompanyId) q = q.neq("id", myCompanyId);
      const { data, error } = await q.order("name", { ascending: true });
      if (error) throw error;

      // Zaten partner olanlar listeden düşsün
      const filtered = (data || []).filter((c) => !existingPartnerIds.has(c.id));
  ...
  // partner listesi güncellenince davet edilebilirler de güncellensin
  useEffect(() => { refreshCompanies(); /* eslint-disable-line react-hooks/exhaustive-deps */ }, [partnerCompanies, myCompanyId]);
  ...
  const handleAddMembers = async () => {
    ...
    try {
      const { error } = await supabase.rpc("add_project_members_bulk", {
        ...
        p_company_id: myCompanyId,
        p_user_ids: selectedMemberIds,
        p_default_role: "calisan",
      });
      if (error) throw error;
      setNotice("Çalışan(lar) projeye eklendi.");
      setSelectedMemberIds([]);
      // listeleri yenile
      await Promise.all([refreshProjectMembers(), refreshMembers()]);
    } catch (e) {
      console.error(e);
      setError(`Çalışan eklenemedi: ${e?.message || e}`);
    } finally {
  ...
  const handleInvitePartners = async () => {
    ...
    // Kendi şirketini ve zaten partner olanları filtrele
    const existingPartnerIds = new Set(partnerCompanies.map((c) => c.id));
    if (myCompanyId) merged = merged.filter((id) => id !== myCompanyId);
    merged = merged.filter((id) => !existingPartnerIds.has(id));
    ...
    try {
      ...
      setNotice(`Partner daveti gönderildi (${merged.length} şirket). Patronların e-posta onayı bekleniyor.`);
      setSelectedCompanyIds([]);
      setManualCompanyIds("");
      // Partnerler değişmiş olabilir → sol/sağ listeleri yenile
      await Promise.all([refreshPartnerCompanies()]);
    } catch (e) {
      console.error(e);
      setError(`Davet gönderilemedi: ${e?.message || e}`);
    } finally {
  ...
  // Sağ sütun (eklenebilir çalışanlar): şirkette olup projede olmayanlar
  const addableMembers = useMemo(
    () => companyMembers.filter((m) => !projectMemberUserIds.has(m.user_id)),
    [companyMembers, projectMemberUserIds]
  );
      ...
      <div className="space-y-6">
        ...
        <Card className="shadow-sm">
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-2 md:col-span-2">
              <Label>Proje Seç</Label>
              <Select value={selectedProjectId} onValueChange={(v) => { setSelectedProjectId(v); setSelectedMemberIds([]); }}>
                <SelectTrigger>
                  <SelectValue placeholder={projectsLoading ? "Yükleniyor…" : "Bir proje seçiniz"} />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="flex items-end">
              <Button variant="outline" onClick={refreshProjects} disabled={projectsLoading} className="w-full md:w-auto">
                {projectsLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <RefreshCcw className="mr-2 h-4 w-4" />}
                Projeleri Yenile
              </Button>
            </div>
          </CardContent>
        ...
        {/* Çalışanlar: iki sütun */}
        <Card className="shadow-sm">
          <CardHeader className="flex items-center gap-2">
            <Users className="h-5 w-5" />
            <CardTitle>Projeye Çalışanlar / Çalışan Ekle</CardTitle>
          </CardHeader>
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Sol: mevcut proje üyeleri */}
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <div className="text-sm text-muted-foreground">
                  Projedeki çalışanlar ({projectMembers.length})
                </div>
                <Button variant="outline" size="sm" onClick={refreshProjectMembers} disabled={projectMembersLoading}>
                  {projectMembersLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <RefreshCcw className="mr-2 h-4 w-4" />}
                  Yenile
                </Button>
              </div>

              {projectMembers.length === 0 ? (
                <div className="text-sm text-muted-foreground">Bu projede şirketinizden aktif çalışan yok.</div>
              ) : (
                <div className="space-y-2">
                  {projectMembers.map((m) => (
                    <div key={m.user_id} className="rounded-xl border p-3">
                      <div className="font-medium">{m.email || m.user_id}</div>
                      <div className="text-xs text-muted-foreground">rol: {m.role} • durum: {m.status}</div>
                    </div>
                  ))}
                </div>
              )}
            ...
            {/* Sağ: eklenebilir çalışanlar */}
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <div className="text-sm text-muted-foreground">
                  Eklenebilir çalışanlar ({addableMembers.length}) <span className="ml-2 opacity-70">(patron listede yok)</span>
                </div>
                <Button variant="outline" size="sm" onClick={refreshMembers} disabled={membersLoading}>
                  {membersLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <RefreshCcw className="mr-2 h-4 w-4" />}
                  Listeyi Yenile
                </Button>
              </div>

              {addableMembers.length === 0 ? (
                <div className="text-sm text-muted-foreground">Eklenecek çalışan bulunamadı.</div>
              ) : (
                <>
                  <div className="space-y-2">
        ...
        {/* Partnerler: iki sütun */}
        <Card className="shadow-sm">
          <CardHeader className="flex items-center gap-2">
            <Send className="h-5 w-5" />
            <CardTitle>Partner Şirketler / Davet Gönder</CardTitle>
          </CardHeader>
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Sol: mevcut partner şirketler */}
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <div className="text-sm text-muted-foreground">
                  Projedeki partner şirketler ({partnerCompanies.length})
                </div>
                <Button variant="outline" size="sm" onClick={refreshPartnerCompanies} disabled={partnersLoading}>
                  {partnersLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <RefreshCcw className="mr-2 h-4 w-4" />}
                  Yenile
                </Button>
              </div>

              {partnerCompanies.length === 0 ? (
                <div className="text-sm text-muted-foreground">Bu projede partner şirket yok.</div>
              ) : (
                <div className="space-y-2">
            ...
            {/* Sağ: davet edilebilir şirketler + manuel ID */}
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="text-sm text-muted-foreground">
                  Davet edilebilir şirketler ({companies.length})
                </div>
                <Button variant="outline" size="sm" onClick={refreshCompanies} disabled={companiesLoading}>
                  {companiesLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <RefreshCcw className="mr-2 h-4 w-4" />}
                  Listeyi Yenile
                </Button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="flex items-end">
                  <Button
                    variant="ghost"
                    onClick={() => { setSelectedCompanyIds([]); setManualCompanyIds(""); }}
                    disabled={selectedCompanyIds.length === 0 && manualCompanyIds.trim() === ""}
                    className="w-full md:w-auto"
                  >
                    Seçimi Temizle
